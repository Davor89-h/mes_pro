...
router.post('/budzet', (req, res) => {
  try {
    ...
    res.json(req.db.get('SELECT * FROM kontroling_budzet WHERE id = ?', [result.lastInsertRowid]))
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.put('/budzet/:id', (req, res) => {
  try {
    ...
    res.json(req.db.get('SELECT * FROM kontroling_budzet WHERE id = ?', [req.params.id]))
  } catch (e) { res.status(500).json({ error: e.message }) }
})

router.delete('/budzet/:id', (req, res) => {
  try {
    req.db.run('DELETE FROM kontroling_budzet WHERE id = ?', [req.params.id])
    res.json({ ok: true })
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.post('/strojni-troskovi', (req, res) => {
  try {
    ...
    res.json(req.db.get('SELECT * FROM kontroling_masinski_sat WHERE id = ?', [result.lastInsertRowid]))
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.put('/strojni-troskovi/:id', (req, res) => {
  try {
    ...
    res.json(req.db.get('SELECT * FROM kontroling_masinski_sat WHERE id = ?', [req.params.id]))
  } catch (e) { res.status(500).json({ error: e.message }) }
})

router.delete('/strojni-troskovi/:id', (req, res) => {
  try {
    req.db.run('DELETE FROM kontroling_masinski_sat WHERE id = ?', [req.params.id])
    res.json({ ok: true })
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.post('/nalog-troskovi', (req, res) => {
  try {
    ...
    res.json(req.db.get('SELECT * FROM kontroling_nalog_troskovi WHERE id = ?', [result.lastInsertRowid]))
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.put('/nalog-troskovi/:id', (req, res) => {
  try {
    ...
    res.json(req.db.get('SELECT * FROM kontroling_nalog_troskovi WHERE id = ?', [req.params.id]))
  } catch (e) { res.status(500).json({ error: e.message }) }
})

router.delete('/nalog-troskovi/:id', (req, res) => {
  try {
    req.db.run('DELETE FROM kontroling_nalog_troskovi WHERE id = ?', [req.params.id])
    res.json({ ok: true })
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.post('/profitabilnost', (req, res) => {
  try {
    ...
    res.json(req.db.get('SELECT * FROM kontroling_profitabilnost WHERE id = ?', [result.lastInsertRowid]))
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.put('/profitabilnost/:id', (req, res) => {
  try {
    ...
    res.json(req.db.get('SELECT * FROM kontroling_profitabilnost WHERE id = ?', [req.params.id]))
  } catch (e) { res.status(500).json({ error: e.message }) }
})

router.delete('/profitabilnost/:id', (req, res) => {
  try {
    req.db.run('DELETE FROM kontroling_profitabilnost WHERE id = ?', [req.params.id])
    res.json({ ok: true })
  } catch (e) { res.status(500).json({ error: e.message }) }
})
...
router.get('/summary', (req, res) => {
  try {
    ...
    const budzet = req.db.all(`SELECT kategorija, SUM(iznos_plan) as plan, SUM(iznos_stvarni) as stvarni FROM kontroling_budzet WHERE godina=? GROUP BY kategorija`, [godina])
    const nalogTroskovi = req.db.all(`SELECT kategorija, SUM(ukupno) as ukupno FROM kontroling_nalog_troskovi GROUP BY kategorija`)
    const profitabilnost = req.db.all(`SELECT SUM(prihod) as prihod, SUM(ukupni_trosak) as trosak, SUM(bruto_dobit) as dobit, AVG(marza_posto) as avg_marza FROM kontroling_profitabilnost WHERE period_god=?`, [godina])
    const strojniSat = req.db.all(`SELECT m.name, ms.trosak_ukupno_sat FROM kontroling_masinski_sat ms LEFT JOIN machines m ON ms.machine_id = m.id ORDER BY ms.vrijedi_od DESC`)
