...
router.get('/roles/all', (req, res) => {
  try {
    const roles = req.db.all('SELECT * FROM roles ORDER BY name')
    const perms = req.db.all('SELECT rp.role_id, p.name FROM role_permissions rp JOIN permissions p ON p.id=rp.permission_id')
...
router.get('/permissions/all', (req, res) => {
  try { res.json(req.db.all('SELECT * FROM permissions ORDER BY module, name')) }
  catch(e) { res.status(500).json({ error: e.message }) }
})
...
router.get('/permissions/my', (req, res) => {
  try {
    if (req.user.role === 'company_admin') {
      return res.json(req.db.all('SELECT name FROM permissions').map(p => p.name))
    }
    const perms = req.db.all(`
...
router.post('/', checkPermission('users.manage'), async (req, res) => {
  try {
    ...
    const r = req.db.prepare('INSERT INTO users (username,password_hash,first_name,last_name,email,role) VALUES (?,?,?,?,?,?)').run(
...
router.get('/:id', (req, res) => {
  try {
    const user = req.db.get('SELECT id,username,first_name,last_name,email,role,active,created_at FROM users WHERE id=?', [req.params.id])
    if (!user) return res.status(404).json({ error: 'Not found' })
    const roles = req.db.all('SELECT r.* FROM roles r JOIN user_roles ur ON ur.role_id=r.id WHERE ur.user_id=?', [req.params.id])
    const perms = req.db.all(`
      SELECT DISTINCT p.name FROM permissions p
      JOIN role_permissions rp ON rp.permission_id=p.id
      JOIN user_roles ur ON ur.role_id=rp.role_id
...
router.put('/:id', (req, res) => {
  try {
    ...
    req.db.prepare('UPDATE users SET first_name=?,last_name=?,email=?,role=?,active=? WHERE id=?').run(
...
router.patch('/:id/password', async (req, res) => {
  try {
    ...
    const hash = await bcrypt.hash(password, 10)
    req.db.prepare('UPDATE users SET password_hash=? WHERE id=?').run(hash, req.params.id)
    res.json({ ok: true })
  } catch(e) { res.status(500).json({ error: e.message }) }
})
...
router.delete('/:id', checkPermission('users.manage'), (req, res) => {
  try {
    ...
    req.db.prepare('UPDATE users SET active=0 WHERE id=?').run(req.params.id)
    res.json({ ok: true })
  } catch(e) { res.status(500).json({ error: e.message }) }
})
...
router.post('/:id/roles', checkPermission('users.manage'), (req, res) => {
  try {
    ...
    const exists = req.db.get('SELECT 1 FROM user_roles WHERE user_id=? AND role_id=?', [req.params.id, role_id])
    if (!exists) req.db.prepare('INSERT INTO user_roles (user_id,role_id,granted_by) VALUES (?,?,?)').run(req.params.id, role_id, req.user.id)
    res.json({ ok: true })
  } catch(e) { res.status(500).json({ error: e.message }) }
})
...
router.delete('/:id/roles/:roleId', checkPermission('users.manage'), (req, res) => {
  try {
    req.db.prepare('DELETE FROM user_roles WHERE user_id=? AND role_id=?').run(req.params.id, req.params.roleId)
    res.json({ ok: true })
  } catch(e) { res.status(500).json({ error: e.message }) }
})
