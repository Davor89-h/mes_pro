...
function ensureTables() {
  req.db.exec(`CREATE TABLE IF NOT EXISTS quality_protocols (
  ...
  req.db.exec(`CREATE TABLE IF NOT EXISTS quality_instruments (
  ...
  req.db.exec(`CREATE TABLE IF NOT EXISTS quality_inspections (
...
router.post('/inspections', (req, res) => {
  try {
    ...
    const r = req.db.prepare(`INSERT INTO quality_inspections (work_order_ref,project_name,type,inspector_id,protocol_id,result,notes) VALUES (?,?,?,?,?,?,?)`
    ).run(work_order_ref||null, project_name||null, type||'završna', req.user.id, protocol_id||null, 'na_čekanju', notes||null)
    res.json(req.db.get('SELECT * FROM quality_inspections WHERE id=?', [r.lastInsertRowid]))
  } catch(e) { res.status(500).json({ error: e.message }) }
})
router.put('/inspections/:id/result', (req, res) => {
  try {
    const { result } = req.body
    req.db.prepare('UPDATE quality_inspections SET result=? WHERE id=?').run(result, req.params.id)
    res.json({ ok: true })
  } catch(e) { res.status(500).json({ error: e.message }) }
})
router.delete('/inspections/:id', (req, res) => {
  try { req.db.prepare('DELETE FROM quality_inspections WHERE id=?').run(req.params.id); res.json({ ok:true }) }
  catch(e) { res.status(500).json({ error: e.message }) }
})

// ── Protocols ──────────────────────────────────────────────────────────────
router.get('/protocols', (req, res) => {
  try { res.json(req.db.all('SELECT * FROM quality_protocols ORDER BY name')) }
  catch(e) { res.status(500).json({ error: e.message }) }
})
router.post('/protocols', (req, res) => {
  try {
    ...
    const r = req.db.prepare('INSERT INTO quality_protocols (name,version,project_name,created_by) VALUES (?,?,?,?)').run(name, version||'1.0', project_name||null, req.user.id)
    res.json(req.db.get('SELECT * FROM quality_protocols WHERE id=?', [r.lastInsertRowid]))
  } catch(e) { res.status(500).json({ error: e.message }) }
})
router.put('/protocols/:id', (req, res) => {
  try {
    const { name, version, status } = req.body
    req.db.prepare('UPDATE quality_protocols SET name=?,version=?,status=? WHERE id=?').run(name, version, status, req.params.id)
    res.json({ ok: true })
  } catch(e) { res.status(500).json({ error: e.message }) }
})
router.delete('/protocols/:id', (req, res) => {
  try { req.db.prepare('DELETE FROM quality_protocols WHERE id=?').run(req.params.id); res.json({ ok:true }) }
  catch(e) { res.status(500).json({ error: e.message }) }
})
...
router.get('/instruments', (req, res) => {
  try {
    ...
    const rows = req.db.all('SELECT *, (next_calibration IS NOT NULL AND next_calibration <= ?) as calibration_due_soon FROM quality_instruments ORDER BY name', [soon])
    res.json(rows)
  } catch(e) { res.status(500).json({ error: e.message }) }
})
router.post('/instruments', (req, res) => {
  try {
    ...
    if (!name) return res.status(400).json({ error: 'Naziv je obavezan' })
    const r = req.db.prepare(`INSERT INTO quality_instruments (name,type,serial_number,manufacturer,storage_location,last_calibration,next_calibration) VALUES (?,?,?,?,?,?,?)`
    ...
    res.json(req.db.get('SELECT * FROM quality_instruments WHERE id=?', [r.lastInsertRowid]))
  } catch(e) { res.status(500).json({ error: e.message }) }
})
router.put('/instruments/:id', (req, res) => {
  try {
    ...
    req.db.prepare('UPDATE quality_instruments SET name=?,type=?,serial_number=?,storage_location=?,last_calibration=?,next_calibration=?,status=? WHERE id=?'
...
router.delete('/instruments/:id', (req, res) => {
  try { req.db.prepare('DELETE FROM quality_instruments WHERE id=?').run(req.params.id); res.json({ ok:true }) }
  catch(e) { res.status(500).json({ error: e.message }) }
})
...
router.get('/stats', (req, res) => {
  try {
    ...
    const cal = req.db.get('SELECT COUNT(*) as calibrations_due FROM quality_instruments WHERE next_calibration IS NOT NULL AND next_calibration <= ?', [soon])
